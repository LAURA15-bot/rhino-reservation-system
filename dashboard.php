...
<head>
    ...
    <!-- Version bump to clear cache and load new Dark Mode UI and Filter logic -->
            ...
            <div class="flex-1 overflow-y-auto p-4 lg:p-6 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 p-3 rounded-2xl custom-shadow border border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row gap-3 items-center transition-colors duration-300">
                    ...
                    <div class="w-full sm:w-2/3 relative flex items-center border border-slate-200 dark:border-slate-600 rounded-xl px-3 bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
                        ...
                        <button onclick="applyLedgerLiveSearchFilters()" class="absolute right-3 text-slate-400 dark:text-slate-500 hover:text-[#046a38] dark:hover:text-emerald-400 transition"><i class="fa-solid fa-magnifying-glass text-sm"></i></button>
                    </div>
                </div>
                ...
                <div id="printable-confirmed-ledger-area" class="bg-white dark:bg-slate-800 rounded-2xl custom-shadow border border-slate-100 dark:border-slate-700 overflow-hidden mt-6 transition-colors duration-300">
                    <div class="p-4 border-b border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 flex justify-between items-center transition-colors duration-300">
                        <h3 class="text-xs font-bold tracking-wide uppercase text-slate-900 dark:text-white flex items-center gap-2"><i class="fa-solid fa-book text-emerald-600 dark:text-emerald-500 text-sm"></i> Booked Entries for this Day (Confirmed)</h3>
                        <div class="flex items-center gap-2">
    ...
    <div id="reservation-modal-backdrop" class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4 overflow-y-auto">
        <div class="bg-white dark:bg-slate-900 w-full max-w-4xl rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 p-6 space-y-5 max-h-[90vh] flex flex-col relative animate-fadeIn transition-colors duration-300">
            ...
            <div class="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3 shrink-0">
                <h2 id="modal-terminal-title" class="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2"><i class="fa-solid fa-calendar-plus text-[#046a38] dark:text-emerald-500"></i> New Reservation Terminal</h2>
            ...
            <div class="shrink-0">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button type="button" id="src-direct-btn" onclick="setBookingSource('direct')" class="py-2.5 px-4 rounded-xl border-2 font-semibold text-xs flex items-center justify-center gap-2 transition-all border-[#046a38] dark:border-emerald-600 bg-emerald-50/50 dark:bg-emerald-900/30 text-[#046a38] dark:text-emerald-400"><i class="fa-solid fa-user"></i> Direct Client</button>
                    <button type="button" id="src-agency-btn" onclick="setBookingSource('agency')" class="py-2.5 px-4 rounded-xl border-2 font-semibold text-xs flex items-center justify-center gap-2 transition-all border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"><i class="fa-solid fa-briefcase"></i> Travel Agency</button>
                </div>
            </div>
            ...
            <div id="agency-fields-wrapper" class="hidden grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shrink-0 transition-colors duration-300">
                <div>
                    ...
                    <input type="text" id="input-agency-name" placeholder="e.g., Perfect Safaris" class="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white rounded-lg p-2 text-xs outline-none focus:ring-2 focus:ring-emerald-500 transition-colors duration-300">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1">Booking Officer *</label>
                    <input type="text" id="input-booking-officer" placeholder="Agent rep name" class="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white rounded-lg p-2 text-xs outline-none focus:ring-2 focus:ring-emerald-500 transition-colors duration-300">
                </div>
            </div>
            ...
            <div class="space-y-2 flex-1 min-h-0 flex flex-col">
                <div class="flex justify-between items-center shrink-0">
                    ...
                    <button type="button" onclick="addNewAllocationRowRow()" class="text-[10px] bg-emerald-50 dark:bg-emerald-900/30 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 text-[#046a38] dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 font-bold py-1 px-2.5 rounded-lg transition-colors flex items-center gap-1"><i class="fa-solid fa-plus"></i> Add Line</button>
                </div>
